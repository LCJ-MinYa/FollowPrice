...
var getProductPrice = function() {
	for (var i = 0; i < Config.PRICE.length; i++) {
		//JD商品
		if (Config.PRICE.url.indexOf('jd.com') > -1) {
			var productId = getJDProductId(Config.PRICE.url);
			request(Config.JD + productId, function(error, response, body) {
				if (!error && response.statusCode == 200) {
					console.log(JSON.parse(body)[0].p);
				} else {
					console.log('请求JD商品' + Config.PRICE.name + '失败', error);
				}
			});
			continue;
		}
		...
		//TMALL商品
		if (Config.PRICE.url.indexOf('tmall.com') > -1) {
			var options = {
